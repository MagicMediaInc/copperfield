              ...
              <?php endif; ?>
				  <article id="post-<?php the_ID(); ?>" <?php post_class(); ?>><?php
					  ...
					  if(isset($_REQUEST['txtcheckindate']) != "" && isset($_REQUEST['txtcheckoutdate']) != ""):
						  $checkin = $_REQUEST['txtcheckindate'];
						  $checkout = $_REQUEST['txtcheckoutdate'];
			  
						  //Putting values to cookies...
						  setcookie('checkin', $checkin, (time()+3600), "/");	setcookie('checkout', $checkout, (time()+3600), "/");
						  setcookie('adults', $_REQUEST['cmbadults'], (time()+3600), "/"); setcookie('childs', $_REQUEST['cmbchilds'], (time()+3600), "/");
						  ...
						  <div class="dt-sc-one-third column first dt-reserve-wrapper">
							  ...
							  <ul>
								  ...
								  <li><i class="fa fa-group"></i><span><?php _e('Guests:', 'iamd_text_domain'); ?> </span><?php echo $_REQUEST['cmbadults']; ?>&nbsp;<?php _e('Adult(s)', 'iamd_text_domain'); ?>, <?php echo $_REQUEST['cmbchilds']; ?>&nbsp;<?php _e('Child(s)', 'iamd_text_domain'); ?></li>
							  ...
							  if($the_query->have_posts()): ?>
								  <ul class="dt-room-list-wrapper"><?php
									  while($the_query->have_posts()): $the_query->the_post();
										  ...
										  $arr = @array_diff($hmeta['room-types'], $unroom_ids[$hid]);
										  $arr = @array_filter($arr);

										  if(array_key_exists($hid, $unroom_ids) && $arr != NULL){
										  	$args = array('post_type' => 'dt_rooms', 'posts_per_page' => -1, 'post__in' => $arr);
										  }
										  else{
										  	$args = array('post_type' => 'dt_rooms', 'posts_per_page' => -1, 'post__in' => array_filter($hmeta['room-types']));
										  }
											  ...
											  while($room_query->have_posts()): $room_query->the_post();
												  ...
												  <li class="dt-room-item">
													  ...
													  <div class="dt-sc-room-meta alignleft">
														  <ul>
															  <li><i class="fa fa-group"></i> <span><?php _e('Occupancy:', 'iamd_text_domain'); ?></span><?php echo $room_meta['room_occupancy']; ?></li>
															  <li><i class="fa fa-building-o"></i> <span><?php _e('Size:', 'iamd_text_domain'); ?></span><?php echo $room_meta['room_size']; ?></li>
														  </ul>
													  </div>
                                                      <div class="alignright">
                                                          <div class="hotel-thumb-meta">
                                                              <div class="hotel-price"><?php _e('Starts From', 'iamd_text_domain'); ?> <span><?php echo dt_theme_currecy_symbol().$room_meta['room_price']; ?></span><?php _e('Per Night', 'iamd_text_domain'); ?></div>
                                                              <form method="post" action="<?php echo $action; ?>" name="frmbook2">
